"use strict";
// Work in progress - making drawing grid isometric / with triangles...
...
// for chat
let folderName = "chat-triangles"; // name of folder you create in db
let messageDiv;
let messageInput;
let usernameInput;
let sendBtn;
...
// for drawing
let drawingNodeID = 'pixelArray-triangle';
let cols = 32;
let rows = 18;
let cellWidth;
let cellHeight;
...
function updateFBPixels() {
  ...
  createNode('drawing', pixelArrayNodeID, pixelArray);
}
...
console.log(_array[i][j]);
      ...
      if (_array[i][j][0].on == true) {
        fill(_array[i][j][0].color);
      } else if (_array[i][j][0].on == false) {
        fill(bgColor);
      }

      rect(x, y, _cellWidth, _cellHeight);
      triangle(x, y, x + _cellWidth, y, x + _cellWidth/2, y + _cellHeight/2)
    }
  }
}
...
// need to check for triangle intersection here
  if (c >= 0 && c < cols && r >= 0 && r < rows && mouseY > 0 && mouseY < height && mouseX > 0 && mouseX < width) {
    if (keyIsDown(SHIFT)) {
      pixelArray[c][r][0].on = false;
    } else {
      pixelArray[c][r][0].on = true;
      pixelArray[c][r][0].color = pixelColorHex;
    }
  }
}
